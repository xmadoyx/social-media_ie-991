...
function tweetReducer(state, action) {
  switch (action.type) {
    ...
    case "TOGGEL_DRAWER":
      return {...state, toggelDrawer : !state.toggelDrawer}
    default: {
...
function useTweetState() {
  ...
  if (context === undefined) {
    throw new Error("useTweettState must be used within a TweetProvider");
  }
  return context;
}
...
export {TweetProvider, useTweetState, useTweetDispatch, setText, likeTweet, setTweetList, setToggelDrawer};

//here is our funtions

function setText(dispatch,tweetText) {
    dispatch({
        type : "SET_TWEET_TEXT",
        payload : tweetText
    });
...
function setToggelDrawer(dispatch) {
  dispatch({
      type : "TOGGEL_DRAWER",
  });
}
